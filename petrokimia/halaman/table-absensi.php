...
?>
			<ol class="breadcrumb bc-3">
				...
				<li class="active">
							...
							<a href="table-absensi.php"><strong>Pengajuan Cuti</strong></a>
					</li>
					</ol><br>
<legend>
<h2>Data Cuti Saya</h2><br>
<p>Yang masih di proses</p>
</legend>
...
<table class="table table-bordered datatable">
	<thead>
		<tr>
			...
			<th>Alasan</th>
			<th>Mulai Tanggal</th>
			<th data-hide="phone,tablet">Sampai Tanggal</th>
			<th data-hide="phone,tablet">Jumlah Hari</th>
	...
	<tbody>
	<?php
		$hasil = $db->QUERY("SELECT * FROM pengajuan_absensi WHERE persetujuan LIKE '%menunggu%' AND nik ='$kd';");
		$rows = $hasil-> fetch_All(MYSQLI_ASSOC);
		$no =1;
		foreach ($rows as $row):
						?>
		...
		<td><?php echo $row['mulai_absensi']; ?></td>
		<td><?php echo $row['akhir_absensi']; ?></td>
		...
		<td><!--<a href="edit_absensi.php?id=<?php echo $row['id'];?>" class="btn btn-info btn-sm btn-icon icon-left">
				...
				<a href="hapus_absensi.php?id=<?php echo $row['id'];?>" class="btn btn-danger	 btn-sm btn-icon icon-left" onclick="return confirm('Apakah Anda yakin ingin menghapus ini?');">
					<i class="entypo-trash"></i>
					Hapus
				</a>
				<a href="draft2.php?id=<?php echo $row['id'];?>" class="btn btn-success btn-sm btn-icon icon-left">
...
<table class="table table-bordered datatable" id="table-1">
	...
	<tbody>
	<?php
		$hasil = $db->QUERY("SELECT * FROM pengajuan_absensi WHERE persetujuan LIKE '%setuju%' AND nik ='$kd';");
		$rows = $hasil-> fetch_All(MYSQLI_ASSOC);
		$no =1;
		foreach ($rows as $row):
						?>
		...
		<td><?php echo $row['persetujuan']; ?></td>
		<td><?php echo $row['keterangan']; ?></td>
					<td><a href="batal_absensi.php?id=<?php echo $row['id'];?>" class="btn btn-warning btn-sm btn-icon icon-left">
