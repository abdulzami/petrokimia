...
<table class="table table-bordered datatable">
	...
	<tbody>
	<?php
		$hasil = $db->QUERY("SELECT * FROM pengajuan_lembur WHERE persetujuan LIKE '%menunggu%' AND nik ='$kd';");
		$rows = $hasil-> fetch_All(MYSQLI_ASSOC);
		$no =1;
		foreach ($rows as $row):
						?>
		...
		<td><a href="edit_lembur.php?id=<?php echo $row['id'];?>" class="btn btn-info btn-sm btn-icon icon-left">
					<i class="entypo-doc-text-inv"></i>
					Edit
				</a>
				<a href="hapus_lembur.php?id=<?php echo $row['id'];?>" class="btn btn-danger	 btn-sm btn-icon icon-left" onclick="return confirm('Apakah Anda yakin ingin menghapus ini?');">
					<i class="entypo-trash"></i>
					Hapus
				</a>
				<a href="draft.php?id=<?php echo $row['id'];?>" class="btn btn-success btn-sm btn-icon icon-left">
...
<table class="table table-bordered datatable" id="table-1">
	...
	<tbody>
	<?php
		$hasil = $db->QUERY("SELECT * FROM pengajuan_lembur WHERE persetujuan LIKE '%setuju%' AND nik ='$kd';");
		$rows = $hasil-> fetch_All(MYSQLI_ASSOC);
		$no =1;
		foreach ($rows as $row):
						?>
		...
		<td><?php echo $row['persetujuan']; ?></td>
		<td><?php echo $row['keterangan']; ?></td>
		<td><a href="batal_lembur.php?id=<?php echo $row['id'];?>" class="btn btn-warning btn-sm btn-icon icon-left">
